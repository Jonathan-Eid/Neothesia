...
fn run_event_loop(event_loop: EventLoop<NeothesiaEvent>) {
...
fn android_main(app: android_activity::AndroidApp) {
    ...
    if let Some(path) = app.internal_data_path() {
        neothesia_core::utils::resources::set_android_files_dir(path);
    } else {
        log::error!("Android app has no internal data path, settings/soundfont won't persist");
    }

    init_logging();

    let event_loop: EventLoop<NeothesiaEvent> = EventLoop::with_user_event()
